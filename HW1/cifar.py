...
for epoch in range(num_epochs):
    ...
    if epoch % 2 == 0:
        single_layer = torch.pca_lowrank(net.conv1.weight)
        for name,param in net.named_parameters():
            if name == 'weight':
                weights_pca.append(torch.pca_lowrank(param))
...
plt.xlabel('epoch')
plt.ylabel('grad')
plt.title('grad vs training')

plt.subplot(1,2,2)
plt.plot(loss_iterations)
plt.xlabel('iteration')
plt.ylabel('loss')
plt.title('loss vs iteations')

plt.show()

plt.subplot(1,2,1)
plt.plot(single_layer[1])

plt.subplot(1,2,2)
plt.plot(weights_pca[1])
